...
#   filename = paste0("Data/ICRATES/time-slices/ICRATES_slice_", i-3, "_range_" ,gsub(",", "to", substr(colnames(df_agg_fabric_loc)[i], 2, nchar(colnames(df_agg_fabric_loc)[i])-1)), ".csv")
...
lvls = NULL
for (i in 4:dim(df_agg_fabric_loc)[2]) {
  lvls = c(lvls, gsub(",", "to", substr(colnames(df_agg_fabric_loc)[i], 2, nchar(colnames(df_agg_fabric_loc)[i])-1)))
}
save(lvls, file = "Data/ICRATES/timeslice_levels.RData")
