---
title: "ICRATES Sensitivity Analysis - Subsample"
author: "Andrew Gillreath-Brown & Emily Coco"
date: "4/21/2021"
...
ICRATES.nets.sub = ICRATES.nets[subsample2]
ICRATES.ta.num.sub = ICRATES.ta.num[subsample2]
...
# Now apply the function to a subsample of networks from the ICRATES dataset.
# BR.rs <- ICRATES.nets %>%
#   purrr::map(~ purrr::map(.x, cv.resamp.bin))
BR.rs = vector(mode = "list", length = length(ICRATES.nets))
exec_time = rep(NA, length(ICRATES.nets))
for (i in 1:length(ICRATES.nets)) {
  start_time = Sys.time()
  list_i = ICRATES.nets[[i]]
  out.list = vector(mode = "list", length = length(list_i))
  for (j in 1:length(list_i)) {
    net_ij = list_i[[j]]
    out.list_ij = cv.resamp.bin(net_ij)
    out.list[[j]] = out.list_ij   
  }
  BR.rs[[i]] = out.list
  names(BR.rs)[i] = names(ICRATES.nets)[i]
  print(i)
  end_time = Sys.time()
  exec_time[i] = difftime(end_time, start_time, units = "mins")
  print(exec_time[i])
}
#names(BR.rs) <- icrates.names 
...
ggsave("../figures/sensitivity-analysis/ICRATES/sensitivity.nodes.plots.subsample.pdf", 
ml.nodes, scale = 1.5)
ml.nodes
...
BR.edge = vector(mode = "list", length = length(ICRATES.nets))
exec_time = rep(NA, length(ICRATES.nets))
for (i in 3:length(ICRATES.nets)) {
  start_time = Sys.time()
  list_i = ICRATES.nets[[i]]
  out.list = vector(mode = "list", length = length(list_i))
  for (j in 1:length(list_i)) {
    net_ij = list_i[[j]]
    out.list_ij = cv.resamp.edge(net_ij)
    out.list[[j]] = out.list_ij   
  }
  BR.edge[[i]] = out.list
  names(BR.edge)[i] = names(ICRATES.nets)[i]
  print(i)
  end_time = Sys.time()
  exec_time[i] = difftime(end_time, start_time, units = "mins")
}
...
# Save as one pdf. Use scale here in order for the multi-plots to fit on each page.
ggsave("../figures/sensitivity-analysis/ICRATES/sensitivity.edges.plots.subsample.pdf", 
ml.edges, scale = 1.5)
ml.edges
