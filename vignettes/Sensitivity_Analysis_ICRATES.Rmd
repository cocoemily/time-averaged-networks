...
load("Data/ICRATES_ta_graph_objects_first-half.RData")
load("Data/ICRATES_ta_graph_objects_second-half.RData")
ICRATES_ta_graphs = append(first_half, second_half)
...
icrates.net.names <- as.data.frame(icrates.names) %>%
  replicate(n = length(ICRATES.nets), simplify = FALSE) %>% 
  unlist() %>% 
  as.data.frame() %>%
  dplyr::rename(period = 1)
icrates.net.names$period <-
  factor(icrates.net.names$period, levels = icrates.names)
icrates.net.names <-
  icrates.net.names[order(icrates.net.names$period), ] %>% 
  as.data.frame() %>% 
  dplyr::rename(period = 1)
icrates.net.names$period <- as.character(icrates.net.names$period)
...
#Next (for additional setup for outputting plots into pdfs), get the total number of networks. Then, create a sequence, which will be used as an index to extract 3 ggplot objects per time period.
total.nets <- length(ICRATES.nets)*length(ICRATES.nets)
middle.net <- round(length(ICRATES.nets)/2)
net.seq <- seq(0, total.nets, by = middle.net)
net.seq[1] <- 1
# Now need to include the first elements throughout the networks so need to add network 21, 41, etc. (which is the original network for all of the subsequent time periods).
net.seq <- as.data.frame(net.seq) %>% 
  dplyr::mutate(other = net.seq +1)
df.new <- net.seq[seq(3, nrow(net.seq), 2), ] %>% 
  dplyr::select(other) %>% 
  dplyr::slice(1:(n()-1)) %>% 
  dplyr::rename(net.seq = other)
# Now combine the two columns and put back into a list.
net.seq.full <- net.seq %>% 
  dplyr::select(net.seq) %>% 
  rbind(df.new) %>% 
  arrange(net.seq) %>% 
  c()
net.seq.full <- net.seq.full[[1]]
...
# Set number of replicates (keeping number low for now, so that it will run quicker, but should definitely increase this number to ?1,000 or ?10,000 for better results).
nsim <- 750
samp.frac <-
...
# Plotting
# First, all the networks are unlisted. The final result is that there is the year of the network (e.g., chaco800), then the final digits are the network number. For example, for the first time averaged network for AD 800 would be listed as "chaco8001", then "chaco8002", and so on, and all the way to "chaco80020". Then, this would continue all the way to the 400th network, which would be "chaco125020". Below, we will rename these, so that this is much clearer.
...
# Arrange each ggplot object to be 1 per page. Use marrangeGrob so that can save each ggplot object to each page, and also put name at top of page. **Need to figure out how to change the plot name for each page.**
...
ggsave("figures/sensitivity-analysis/ICRATES/sensitivity.nodes.plots.pdf", ml.nodes, scale = 1.5)
plot(ml.nodes)
...
# Save as one pdf. Use scale here in order for the multi-plots to fit on each page.
ggsave("figures/sensitivity-analysis/ICRATES/sensitivity.edges.plots.pdf", ml.edges, scale = 1.5)
