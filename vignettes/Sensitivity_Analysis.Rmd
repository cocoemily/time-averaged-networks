...
total.nets <- length(Chaco.nets)*length(Chaco.nets)
middle.net <- round(length(Chaco.nets)/2)
...
# This outputs a list of ggplot objects for all the time averaged networks for Chaco, using the plot_sa function (in the R folder).
...
names(plots_sa_nodes) <- c(chaco.net.names.full$period_network)
...
chaco.net.names.full.subset <- chaco.net.names.full[c(net.seq.full),]
...
ml.nodes <- gridExtra::marrangeGrob(plots_subset_nodes, nrow=1, ncol=1)

# Save as one pdf. Use scale here in order for the multi-plots to fit on each page.
ggsave("figures/sensitivity-analysis/Chaco/sensitivity.nodes.plots.pdf", ml.nodes, scale = 1.5)
...
ml.edges <- gridExtra::marrangeGrob(plots_subset_edges, nrow=1, ncol=1, top = quote(chaco.net.names.full.subset))

# Save as one pdf. Use scale here in order for the multi-plots to fit on each page.
ggsave("figures/sensitivity-analysis/Chaco/sensitivity.edges.plots.pdf", ml.edges, scale = 1.5)
