...
load("../Data/Chaco_ta_graph_objects_1.RData")
load("../Data/Chaco_ta_graph_objects_2.RData")
load("../Data/Chaco_ta_graph_objects_3.RData")

Chaco_ta_graphs = append(append(cf, cs), ct)
...
names = c()
for(i in 1:length(Chaco.nets)) {
  names = c(names, rep(names(Chaco.nets)[i], each=length(Chaco.nets[[i]])))
}
chaco.net.names = as.data.frame(names) %>% dplyr::rename(period = 1)
...
save(chaco.net.names.full, file="Data/chaco_network_names.RData")
...
saveRDS(BR.rs.plotting, file = "Data/sensitivity_analysis_nodes_Chaco.rds")
...
# Then, we can rename each plot to have the number of graphs that are time averaged along with year (e.g., chaco800_network_1).
...
ggsave("../figures/sensitivity-analysis/Chaco/sensitivity.nodes.plots.pdf", 
ml.nodes, scale = 1.5)
ml.nodes
...
saveRDS(BR.edge.plotting, file = "Data/sensitivity_analysis_edges_Chaco.rds")
...
# Save as one pdf. Use scale here in order for the multi-plots to fit on each page.
ggsave("../figures/sensitivity-analysis/Chaco/sensitivity.edges.plots.pdf", 
ml.edges, scale = 1.5)
ml.nodes
