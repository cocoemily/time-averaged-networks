...
# Run function on binary network. This line was just for testing to focus on 1 network.
...
test.BR.rs = BR.rs$chaco800[[1]]
#test.BR.rs = BR.rs[c(1,2) ]
#plot_node_removal_sensitivity = function(test.BR.rs) {
...
#plot_node_removal_sensitivity(BR.rs$chaco800[[1]])
...
# Now apply the function to all 400 networks (20 per time period) for Chaco.
BR.edge <- Chaco.nets %>% purrr::map(~ purrr::map(.x, cv.resamp.edge))

# Run the script for the binary network. his line was just for testing to focus on 1 network.
...
# Setup modularity and clustering coefficient for plotting. 
test.BR.edge = BR.edge$chaco800[[1]]

cc.mat.long <- as.data.frame(test.BR.edge[[4]]) %>%
  tidyr::gather(
    "S100",
    "S90",
    "S80",
...
mod.mat.long <- as.data.frame(test.BR.edge[[5]]) %>%
  tidyr::gather(
    "S100",
    "S90",
    "S80",
...
#Plot the results. This will also need to be updated to run on more than one network. Will need to make this a function, then use map to output for each network (or a sample of the networks).
par(mfrow = c(2, 3))
boxplot(test.BR.edge[[1]],
        main = "BR - degree",
        xlab = "sampling fraction",
        ylab = "Spearmans rho")
boxplot(test.BR.edge[[2]], main = "BR - eigenvector", xlab = "sampling fraction")
boxplot(test.BR.edge[[3]],  main = "BR - betweenness", xlab = "sampling fraction")
plot(
  cc.mat.long$sample.perc,
  cc.mat.long$cc.score,
  type = "l",
