...
net.stats.igraph <- function(y) {
    ...
    bw <- igraph::betweenness(y, normalized = TRUE, directed = TRUE)
...
resamp.test <- function(x) {
    ...
    for (j in 1:9) {
        for (i in 1:nsim) {
            ...
            temp.stats <- igraph::betweenness(igraph::graph_from_adjacency_matrix(mat[sub.samp, sub.samp], gmode = "graph"))
