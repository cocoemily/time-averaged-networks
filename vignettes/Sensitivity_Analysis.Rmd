...
# Now, arrange multiple plots on each page. 
#First, get the total number of networks (for chaco 20 time averaged networks per time period for total of 400). Then, create a sequence, which will be used as an index to extract 3 ggplot objects per time period.
total.nets <- length(Chaco.nets)*length(Chaco.nets)
middle.net <- round(length.nets/2)
...
# Get just the plots that we want to output.
plots_subset <- plots_sa_nodes[c(net.seq.full)]


# multi.page <- ggpubr::ggarrange(ok3[[1]], ok3[[2]], ok3[[3]], ok3[[4]], ok3[[5]], ok3[[6]],
#                         nrow = 1, ncol = 1)

#Need to adjust original plots, as test is overlapping and spilling over the edge of the margins.
#ggexport(ok3[[1]], filename = "/Users/andrewgillreath-brown/Desktop/multi.page.ggplot2.pdf")


# sublist.names <- names(plots_sa_nodes)
# sublist.names <- gsub("_.*$", "", sublist.names)
# sublist.names <- unique(sublist.names)
