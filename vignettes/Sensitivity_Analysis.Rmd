...
# For simplicity sake (of adapting code), both igraph and network objects will be used via intergraph conversion for this analysis.
...
resamp.test <- function(x.igraph, x.network) {
    # Can do any centrality measure here (i.e., doesn't have to be betweenness).
    bw.g <- igraph::betweenness(x.igraph, normalized = TRUE, directed = FALSE)
    mat <- as.matrix(x.network)
    dim.x <- dim(mat)[1]
...
boxplot(resamp.test(x.igraph = tn, x.network = tn2), main = "BR - betweenness", xlab = "sampling fraction", 
    ylab = "Spearmans rho")
