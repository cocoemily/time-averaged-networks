...
net.stats <- function(y) {
    ...
    output <- cbind(dg, eg, bw, cc, mod)
    colnames(output) <- c("dg", "eg", "bw", "cc", "mod")
    return(output)
...
# This double loop goes through each sampling fraction and each random replicate to calculate centrality statistics and runs a Spearman's Rho correlation between the resulting centrality values and the original sample.
    for (j in 1:9) {
        for (i in 1:nsim) {
            sub.samp <- sample(seq(1, dim.x), size = round(dim.x * ((10 - 5)/10), 
                0), replace = F)
            ...
            bw.mat[i, j] <- suppressWarnings(cor(temp.stats[, 3], stats.g[sub.samp, 
                3], method = "spearman"))
            cc.mat[1, j] <- suppressWarnings(cor(temp.stats.single[, 1], stats.g.single[1, 
                1], method = "spearman"))
            mod.mat[i, j] <- suppressWarnings(cor(temp.stats.single[, 2], stats.g.single[1, 
                2], method = "spearman"))
        }
    }
